...
def add(A, B):
    ...
    matrixSum = [[sum(x) for x in zip(A.store[i], B.store[i])] for i in range(len(A.store))]
...
def scalar_mul(M, x):
    ...
    True
    """
    multiplied = [[a * x for a in M.store[i]] for i in range(len(M.store))]
...
def transpose(M):
    ...
    True
    """
    transMatrix = [[a[i] for a in M.store] for i in range( len(M.store[0]))]
    return Mat(transMatrix)
