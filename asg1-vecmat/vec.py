...
def add(u, v):
    ...
    vectorSum = [a + b for a, b in zip(u.store, v.store)]
    return Vec(vectorSum)
...
def dot(u, v):
    ...
    dotProduct = sum([a * b for a, b in zip(u.store, v.store)])
    return dotProduct
...
def scalar_mul(v, alpha):
    ...
    True
    """
    multiplied = [a * alpha for a in v.store]
...
def neg(v):
    ...
    True
    """
    vectorNeg = [-a for a in v.store]
    return Vec(vectorNeg)
