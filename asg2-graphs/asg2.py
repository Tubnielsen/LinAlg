...
def isPermutationMatrix(matrix):
    ...
    if type(matrix) is np.ndarray:
        for summedColumn in np.sum(matrix, axis=0):
            if summedColumn != 1:
                return False
        
        for summedRow in np.sum(matrix, axis=1):
            if summedRow != 1:
                return False
        return True
    return False
...
def isIsomorphicUsingP(A, B, P):
    ...
    if isPermutationMatrix(P) != True:
        raise TypeError
    
    if np.array_equal(A, (np.matmul(P,np.transpose(np.matmul(P,B))))):
        return True
